...
export default function GuestDashboard({ onBack }: GuestDashboardProps) {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', height: '100vh', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: '20px', marginBottom: '15px', padding: '10px 15px', backgroundColor: '#f8f9fa', borderRadius: '6px', border: '1px solid #dee2e6', alignItems: 'center' }}>
          <span style={{ fontWeight: 'bold', fontSize: '0.9em', color: '#495057' }}>Filter Status:</span>
          {Object.keys(statusFilters).map(status => (
            <label key={status} style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '0.9em' }}>
              <input
                type="checkbox"
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: '5px' }}>
        <GuestGrid guests={filteredGuests} onRowClick={(guest) => { setSelectedGuest(guest); setIsDrawerOpen(true); }} checkOutTime={checkOutTime} />
      </div>
