...
export default function GuestForm({ onClose, onSuccess }: GuestFormProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        backgroundColor: 'white', padding: '30px', borderRadius: '8px', width: '500px',
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div>
            ...
            <input required type="text" name="guestName" value={formData.guestName} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
          </div>
          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            <div style={{ flex: 1 }}>
              ...
              <input required type="date" name="checkInDate" value={formData.checkInDate} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
            </div>
            <div style={{ flex: 1 }}>
              <label>Check-Out Date *</label>
              <input required type="date" name="checkOutDate" value={formData.checkOutDate} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            <div style={{ flex: 1 }}>
              ...
              <input type="number" step="0.01" name="totalAmount" value={formData.totalAmount} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
            </div>
            <div style={{ flex: 1 }}>
              <label>Amount Paid ($)</label>
              <input type="number" step="0.01" name="amountPaid" value={formData.amountPaid} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
            </div>
          </div>

          <div>
            <label>Transaction Status</label>
            <input type="text" name="paymentStatus" placeholder="e.g., Cash, EVC, Pending..." value={formData.paymentStatus} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
            ...
            <button type="submit" style={{ padding: '8px 16px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Save Guest</button>
          </div>
        </form>
      </div>
    </div>
